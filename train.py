...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--config",
        type=str,
        default="config/framework_train.json",
    ...
    logger.info(
        "change rate:"
        + "".join(["{}:{} ".format(k, v) for k, v in change_sizes.items()])
    )
    ...
    if current_step == 0:
        ...
        try:
            while current_step >= int(
                float(list(change_sizes.keys())[change_size_idx]) * n_iter
            ) and change_size_idx < len(list(change_sizes.keys())):
                change_size_idx += 1
        except:
            pass
        change_size_idx -= 1
    ...
    while current_step < n_iter:
        ...
        if current_step >= int(
            ...
            logger.info(
                "reset train_loader: l_resolution:{}, r_resolution:{}, batch_size:{}".format(
                    train_dataset_opt["l_resolution"],
                    train_dataset_opt["r_resolution"],
