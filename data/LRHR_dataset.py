...
class LRHRDataset(Dataset):
    def __init__(
        ...
        self.down_sample = (
            other_params["down_sample"]
            if "down_sample" in other_params.keys()
            else None
        )
        self.real_hr_path = (
        ...
        if datatype in ["haze_img"]:
            ...
            self.dis_prefix = (
                other_params["distanse_prefix"]
                if "distanse_prefix" in other_params.keys()
                else None
            )
            if self.data_len <= 0:
    ...
    def __getitem__(self, index):
        ...
        if self.datatype in ["RESIDE_img_syntheic"]:
            ...
            if self.rt_da:
                ...
                img_SR = rt_haze_enhancement(
                    ...
                    os.path.join(
                        self.depth_path,
                        "{}_depth.png".format(
                            self.sr_path[index].split("/")[-1].split(".")[0]
                        ),
                    ),
            ...
            # hr_path
            hr_path = "{}/{}".format(
                self.real_hr_path, self.sr_path[index].split("/")[-1]
            )
    ...
    def resize(self, input_image):
        ...
        resize_H, resize_W = neibor_16_mul(int(H / self.down_sample)), neibor_16_mul(
            int(W / self.down_sample)
        )
