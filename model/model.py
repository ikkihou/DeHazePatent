...
class DDPM(BaseModel):
    def __init__(self, opt):
        ...
        # ema
        self.use_ema = (
            opt["train"]["ema_scheduler"]["used"]
            if "used" in opt["train"]["ema_scheduler"].keys()
            else False
        )
        if self.use_ema:
            self.decay = opt["train"]["ema_scheduler"]["ema_decay"]
            self.ema_start = opt["train"]["ema_scheduler"]["step_start_ema"]
            self.shadow = {}
            self.backup = {}
            self.register()
            print("using ema to training ...")
    ...
    def optimize_parameters(self, current_step):
        ...
        if self.use_ema and current_step > self.ema_start:
            self.update()
