...
class GaussianDiffusion(nn.Module):
    ...
    def set_loss(self, device):
        ...
        self.optim_loss = nn.MSELoss(reduction="sum").to(device)
    ...
    def p_sample_loop(self, x_in, continous=False):
        ...
        if condition_ddim:
            ...
            len_seq = len(seq)
            for idx, (i, j) in tqdm(
                enumerate(zip(reversed(seq), reversed(seq_next))),
                ...
                noise_level = (
                    torch.FloatTensor([self.sqrt_alphas_cumprod_prev[i + 1]])
                    .repeat(batch_size, 1)
                    .to(x.device)
                )
            ...
            if not self.conditional:
                ...
                x_in_numpy = (x_in_numpy - np.min(x_in_numpy)) / (
                    np.max(x_in_numpy) - np.min(x_in_numpy)
                )
    ...
    def calc_RGB(self, tensor):
        b, c = tensor.shape[:2]
        RGB_mean = torch.mean(tensor.view(b, c, -1), -1)
        return RGB_mean
